...
function Frame({ eyebrow, title, goal, children }: { eyebrow: string; title: string; goal: string; children: React.ReactNode }) {
  return <section className="archive-game"><header><p>{eyebrow}</p><h1>{title}</h1><strong>OBJECTIVE · {goal}</strong></header>{children}</section>
}
...
export function RunDarkGame({ onComplete }: { onComplete: (result: ArchiveGameResult) => void }) {
  ...
  return <Frame eyebrow="BEAT 13 · DEATH PROTOCOL" title="Silence the living ship" goal="Shut four signatures down without leaving the crew unconscious before navigation goes dark.">
...
export function GateEvidenceGame({ onComplete }: { onComplete: (result: ArchiveGameResult) => void }) {
  ...
  return <Frame eyebrow="BEAT 14 · ARCHIVE TESTIMONY" title="Reconstruct the crime before memory becomes argument" goal="Order four records by cause—not by the official timestamps that were rewritten.">
...
export function DroneMemoryGame({ onComplete }: { onComplete: (result: ArchiveGameResult) => void }) {
  ...
  return <Frame eyebrow="BEAT 15 · UNBURIED SIGNAL" title="Separate a person from the weapon still obeying for them" goal="Recover four human memory anchors without reconnecting the drone’s fire-control compulsion.">
...
export function MessageAssemblyGame({ onComplete }: { onComplete: (result: ArchiveGameResult) => void }) {
  ...
  return <Frame eyebrow="BEAT 16 · EARTH CARRIER" title="Bring one message home from the noise" goal="Align the four damaged signal bands while preserving gaps the Archive cannot honestly reconstruct.">
...
export function ProbabilityGame({ onComplete }: { onComplete: (result: ArchiveGameResult) => void }) {
  ...
  return <Frame eyebrow="BEAT 17 · TIRESIAS" title="Find the future that does not contradict itself" goal="Select the three events that can share one route; direct Earth is the comforting impossibility.">
